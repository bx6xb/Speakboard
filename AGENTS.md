...
| `logger.py` | Startup/runtime logging to `speakboard.log` (gitignored) |
...
## Logging (`speakboard.log`)

- Written next to `main.py` on every launch. Gitignored (`*.log`).
- Records startup steps (GPU, model load, autostart, Ready), warnings, and errors.
- Rotates to `speakboard.log.old` when the file exceeds 512 KB.
- With `pythonw` / autostart there is no console — the log file is the only trace.
- **When the user reports a startup or runtime problem**, read `speakboard.log` first.
  If the file is missing or ends before `[App] Ready`, the app crashed or never started.
  Check the last lines and any `ERROR` / traceback entries.
